...
export default function Home() {
    ...
    function handleClick() {
        ...
        let mainArr = ["XS", "S", "M", "L", "XL", "XXL"];
        let sizesArr = []; // array of sizes which is checked
        let indexArray = []; // array of indexes where are same sizes in data

        for (let i = 1; i <= 6; i++) {
            if (document.getElementById('check_' + i).checked) {
                sizesArr.push(mainArr[i - 1]);
            }
        }

        for (let j = 0; j < data.length; j++) {
            if (includesOrNot(sizesArr, data[j].availableSizes)) {
                indexArray.push(j);
            }
        }
    ...
    return (
        <div>
            ...
            <div id="render">
                {Data.map((item) => {
                    return <ItemRender key={item.id} item={item}/>
                })}
            </div>
        </div>
    );
